...
parser.add_argument('--lr', default=0.1, type=float) #default=0.1
...
def main():
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_loss, val_acc = evaluate(model, test_loader, device, args)
...
def train(model, train_loader, loss_fn, optimizer, epoch, device, scaler, writer, args):
    ...
    for i, data in enumerate(train_loader):
        ...
        #                 labels_a, classes=5, smoothing=args.label_smooth_value)
        #             soft_labels_b = smooth_one_hot(
        #                 labels_b, classes=5, smoothing=args.label_smooth_value)
        #             loss = mixup_criterion(
        #                 loss_fn, outputs, soft_labels_a, soft_labels_b, lam)
        #         else:
        #             # label smoorth
        #             soft_labels = smooth_one_hot(
        #                 labels, classes=5, smoothing=args.label_smooth_value)
