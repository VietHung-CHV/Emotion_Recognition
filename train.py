...
def run(net, logger, hps):
    # Create dataloaders
    trainloader, valloader, testloader = get_dataloaders(path='datasets/fer2013/fer2013_5emo.csv', bs=hps['bs'])
    ...
    best_acc = 0.0
    early_stopping = EarlyStopping(tolerance=5, min_delta=0.015)
    ...
    for epoch in range(hps['start_epoch'], hps['n_epochs']):
        ...
        print('Epoch %2d' % (epoch + 1),
              ...
              'Train Loss: %.4f ' % loss_tr,
              'Val Loss: %.4f ' % loss_v,
              sep='\t')
        
        # early stopping
        # early_stopping(loss_tr, loss_v)
